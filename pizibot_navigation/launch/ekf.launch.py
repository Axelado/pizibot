...
def generate_launch_description():
    ...
    # The simulated diff drive plugin publishes on /odom directly, while the
    # real diff_drive_controller publishes on /diff_drive_controller/odom.
    declare_odom_topic_cmd = DeclareLaunchArgument(
        'odom_topic',
        default_value=PythonExpression(
            ["'/odom' if '", use_sim_time, "' == 'true' else '/diff_drive_controller/odom'"]
        ),
        description='Wheel odometry topic fused by the EKF (odom0). Empty: derived from use_sim_time'
    )
