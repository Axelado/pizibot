...
- ⏳ **Pending Migration**:
  - `pizibot_navigation` - SLAM and Nav2
...
#### 1. **pizibot_navigation** - SLAM & Path Planning
- **Status**: Built for ROS 2 Humble
- **What works**: Core navigation and mapping functionality exists
- **What's missing**: 
  - Compatibility with ROS 2 Jazzy Nav2 stack
  - Updated SLAM Toolbox bindings for Jazzy
  - Testing in Gazebo Harmonic environment
- **Workaround**: Can run on Humble, but integration with Jazzy simulation is not yet tested

#### 2. **pizibot_hardware** - Real Robot Interface
- **Status**: Built for ROS 2 Humble
- **Features developed**:
...
#### 3. **pizibot_vision** - Computer Vision
...
- **Complete Autonomous Navigation**: SLAM and Nav2 not yet tested with Gazebo Harmonic
...
- **🎮 For Simulation Testing**: Use `pizibot_description`, `pizibot_gz`, and `pizibot_teleop` (all Jazzy-ready)
- **🤖 For Real Robot**: Still requires ROS 2 Humble environment until `pizibot_hardware` is migrated
- **🗺️ For Navigation/SLAM**: Use Humble distribution until `pizibot_navigation` is migrated
